...
interface FilteredChatBubblesProps {
	maxHeight?: string; // e.g., "300px", "60vh", or undefined for flex-1
	className?: string; // Additional classes for the container
}

export const FilteredChatBubbles: React.FC<FilteredChatBubblesProps> = ({
	maxHeight,
	className = '',
}) => {
	...
	const visibleMessages = messages.filter(message => 
		message.role === 'user' || message.role === 'assistant' || message.role === 'bot'
	);
	...
	const isConsecutiveMessage = (index: number): boolean => {
		...
		return visibleMessages[index].role === visibleMessages[index - 1].role;
	};
...
export default FilteredChatBubbles;
